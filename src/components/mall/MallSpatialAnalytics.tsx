...
export function MallSpatialAnalytics() {
  ...
  return <div className="space-y-5">
    ...
    <p className="px-1 text-[9px] leading-relaxed text-[var(--mn-faint)]">Observed metrics, audience segments, catchment, rhythm, cross-shopping and audience drift are from VectorMobility’s July 2026 Singapore Veraset panel and are not absolute mall totals. SES, behavior segments and trip-chain allocations are illustrative, modeled signals. Indoor zone metrics, journey funnels and—for {profile.planKind === 'schematic' ? 'this mall' : 'the Wi‑Fi layer'}—the floor plan itself are synthetic demonstrations informed by broader July behavior and public campus geometry; tenant and layout details should be sense-checked with the operator before external circulation.</p>
  </div>
}
